...
serve(async (req) => {
  ...
  try {
    ...
    if (producer?.spotify_url)
      producerSocialLinks.push(
        `<a href="${producer.spotify_url}" style="display: inline-block; margin: 0 8px; color: #1DB954; text-decoration: none; font-size: 14px;">🎧 Spotify</a>`,
      );
    if (producer?.instagram_url)
      producerSocialLinks.push(
        `<a href="${producer.instagram_url}" style="display: inline-block; margin: 0 8px; color: #E4405F; text-decoration: none; font-size: 14px;">📸 Instagram</a>`,
      );
    if (producer?.youtube_url || producer?.youtube_channel_url)
      producerSocialLinks.push(
        `<a href="${producer.youtube_url || producer.youtube_channel_url}" style="display: inline-block; margin: 0 8px; color: #FF0000; text-decoration: none; font-size: 14px;">▶️ YouTube</a>`,
      );
    if (producer?.apple_music_url)
      producerSocialLinks.push(
        `<a href="${producer.apple_music_url}" style="display: inline-block; margin: 0 8px; color: #FA243C; text-decoration: none; font-size: 14px;">🍎 Apple Music</a>`,
      );
    if (producer?.website_url)
      producerSocialLinks.push(
        `<a href="${producer.website_url}" style="display: inline-block; margin: 0 8px; color: #7C3AED; text-decoration: none; font-size: 14px;">🌐 Website</a>`,
      );
