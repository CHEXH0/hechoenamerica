...
serve(async (req) => {
  ...
  try {
    ...
    if (producer.spotify_url) {
      socialLinks.push(
        `<a href="${producer.spotify_url}" style="display: inline-block; padding: 8px 16px; background: #1DB954; color: white; text-decoration: none; border-radius: 20px; font-size: 13px; font-weight: 600;">🎧 Spotify</a>`,
      );
    }
    if (producer.apple_music_url) {
      socialLinks.push(
        `<a href="${producer.apple_music_url}" style="display: inline-block; padding: 8px 16px; background: #FC3C44; color: white; text-decoration: none; border-radius: 20px; font-size: 13px; font-weight: 600;">🎵 Apple Music</a>`,
      );
    }
    if (producer.instagram_url) {
      socialLinks.push(
        `<a href="${producer.instagram_url}" style="display: inline-block; padding: 8px 16px; background: linear-gradient(45deg, #f09433, #e6683c, #dc2743, #cc2366, #bc1888); color: white; text-decoration: none; border-radius: 20px; font-size: 13px; font-weight: 600;">📸 Instagram</a>`,
      );
    }
    if (producer.youtube_url || producer.youtube_channel_url) {
      socialLinks.push(
        `<a href="${producer.youtube_url || producer.youtube_channel_url}" style="display: inline-block; padding: 8px 16px; background: #FF0000; color: white; text-decoration: none; border-radius: 20px; font-size: 13px; font-weight: 600;">▶ YouTube</a>`,
      );
    }
    if (producer.website_url) {
      socialLinks.push(
        `<a href="${producer.website_url}" style="display: inline-block; padding: 8px 16px; background: #27272a; color: white; text-decoration: none; border-radius: 20px; font-size: 13px; font-weight: 600; border: 1px solid #3f3f46;">🌐 Website</a>`,
      );
    }

    // Build showcase videos section
    const showcaseVideos = [producer.showcase_video_1, producer.showcase_video_2, producer.showcase_video_3].filter(
      Boolean,
    );
