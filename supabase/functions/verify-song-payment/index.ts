...
serve(async (req) => {
  ...
  try {
    ...
    if (addOns.number_of_revisions > 0)
      addOnsList.push(`${addOns.number_of_revisions} Revision${addOns.number_of_revisions > 1 ? "s" : ""}`);
        ...
        html: `
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8f9fa;">
            ...
            <div style="text-align: center; padding: 24px; color: #999; font-size: 13px;">
              ...
              <p style="margin: 0;"><strong>LA MÚSICA ES MEDICINE</strong></p>
    ...
    if (requestId) {
      ...
      const { error: updateError } = await supabaseAdmin.from("song_requests").update(updateData).eq("id", requestId);
      ...
      const { error: insertError } = await supabaseAdmin.from("song_requests").insert({
        user_id: userId,
